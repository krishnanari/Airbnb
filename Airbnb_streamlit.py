...
if select == "Data Exploration":
    ...
    with tab1:
        ...
        with col1:
            ...
            country= st.selectbox("Select the Country",df["country_x"].unique())

            df1= df[df["country_x"] == country]
    ...
    with tab2:
        ...
        def datafr():
            df_a = pd.read_csv(r"C:\Users\USER\Downloads\New folder\Airbnb.csv")
            return df_a
        ...
        with col1:
            ...
            country_a= st.selectbox("Select the Country_a",df_a["country_x"].unique())

            df1_a= df[df["country_x"] == country_a]
            ...
            df_a_sunb_30= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact_x"], values="availability_30",width=600,height=500,title="Availability_30",color_discrete_sequence=px.colors.sequential.Peach_r)
        ...
        with col2:
            ...
            df_a_sunb_60= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact_x"], values="availability_60",width=600,height=500,title="Availability_60",color_discrete_sequence=px.colors.sequential.Blues_r)
        ...
        with col1:
            ...
            df_a_sunb_90= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact_x"], values="availability_90",width=600,height=500,title="Availability_90",color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
            st.plotly_chart(df_a_sunb_90)

        with col2:

            df_a_sunb_365= px.sunburst(df2_a, path=["room_type","bed_type","is_location_exact_x"], values="availability_365",width=600,height=500,title="Availability_365",color_discrete_sequence=px.colors.sequential.Greens_r)
    ...
    with tab3:
        ...
        country_l= st.selectbox("Select the Country_l",df_l["country_x"].unique())

        df1_l= df_l[df_l["country_x"] == country_l]
        ...
        # Check for missing or invalid values in numeric columns
        st.write(df_val_sel.select_dtypes(include=[np.number]).isnull().sum())

        # Calculate correlation matrix
        try:
            df_val_sel_corr = df_val_sel.drop(columns=["listing_url", "name", "property_type", "room_type", "bed_type", "cancellation_policy", "host_url_y", "host_name_x", "host_location", "host_response_time", "host_thumbnail_url", "host_response_rate", "host_is_superhost", "host_has_profile_pic", "host_picture_url", "host_neighbourhood", "host_identity_verified", "host_verifications", "street_x", "suburb", "government_area", "market", "country_x", "country_code_x", "location_type_x", "is_location_exact_x", "amenities"]).corr()
    ...
    with tab4:
        ...
        fig_4 = px.scatter_mapbox(df, lat='latitude_x', lon='longitude_x', color='price', size='accommodates',
                          hover_data=['name', 'price', 'accommodates'],
    ...
    with tab5:
        ...
        country_t= st.selectbox("Select the Country_t",df["country_x"].unique())

        df1_t= df[df["country_x"] == country_t]
